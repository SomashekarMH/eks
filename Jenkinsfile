pipeline{
    parameters {name:'terraform_command', choices: ['plan', 'apply', 'destroy'], description: 'Select the Terraform command to execute.'    
    }
        ...
        stage('approve') {
            steps {
                script {
                    ...
                    input message: "Terraform Plan:\n${plan}\n\nDo you want to apply this plan?", ok: 'Apply'
                    parameters: [text(name: 'approval', defaultValue: 'yes', description: 'Type "yes" to approve the plan.')]
                    }
                }
            }
        stage(apply or destroy) {
            when{
                expression { return params.terraform_command == 'apply' || params.terraform_command == 'destroy' }
            }
        }
            steps{
                scripr{
